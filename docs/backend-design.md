...
8. 同步传感器屏蔽位图到 ESP32
9. 启动 30s 数据采集定时器
10. 启动实时推送队列 (采集后立即推送数据至前端，延迟显示由前端实现)
11. 启动灌溉任务调度循环
12. 启动数据清理定时器 (每日凌晨执行)
...
| `calib_a`    | FLOAT (nullable) | 校准参数 a（对数回归斜率）    |
| `calib_b`    | FLOAT (nullable) | 校准参数 b（对数回归截距）    |
| `created_at` | DATE             | 添加时间                      |

> 含水量转换（校准完成后启用）：利用 `calib_a` 和 `calib_b`，按公式 $y = a \ln(\frac{1000}{x}) + b$ 将脉冲计数转换为含水量。
...
用户收集至少 2 个数据点后，可请求计算校准公式。后端使用最小二乘法进行**对数回归**拟合，将结果写入 `Sensor.calib_a` 和 `Sensor.calib_b`，并设置 `calibrated = true`。

##### 校准公式

通过实验测定，含水量 $y$ 与脉冲计数 $x$ 之间满足**对数关系**：

$$
y = a \ln\!\left(\frac{1000}{x}\right) + b
$$

其中 $a$ 为斜率（`calib_a`），$b$ 为截距（`calib_b`）。

**拟合方法**：令 $u = \ln\!\left(\frac{1000}{x}\right) = \ln(1000) - \ln(x)$，将公式线性化为 $y = a \cdot u + b$，对变换后的数据点 $(u_i, y_i)$ 使用标准最小二乘法求解 $a$ 和 $b$。

**含水量转换**（校准完成后启用）：

$$
\text{moisture} = \text{calib\_a} \cdot \ln\!\left(\frac{1000}{\text{pulse\_count}}\right) + \text{calib\_b}
$$

> **约束**：`pulse_count` 必须 $> 0$ 且 $< 1500$ 方可进行转换。若 `pulse_count = 0`（无脉冲）或 $\geq 1500$（超出预期范围），含水量返回 `null`，该传感器不参与平均含水量计算。
>
> 注：公式中的常数 $1000$ 是拟合参数，与有效脉冲计数范围无关。干燥土壤中脉冲计数约 1200，仍处于有效范围内（$< 1500$），此时 $\ln(\frac{1000}{1200}) < 0$，属于正常计算结果。
>
> **关于含水量超出 [0%, 100%] 范围**：校准以传感器探头暴露在空气中作为含水量 0% 的基准、浸泡在水中作为 100% 的基准。在潮湿土壤中，受电解质影响，按公式计算出的含水量可能略微超过 100%；干燥空气中也可能略低于 0%。**参与平均含水量计算时，超出范围的数值截断到 [0, 100]**，避免个别传感器的越界值拉偏平均值。传感器明细（`RawSensorReading.moisture`）仍保留原始计算值，不截断。
...
sequenceDiagram
    ...
    DATA->>QUEUE: 入队并立即 emit: DataSnapshot
...
> **关键变更**：数据采集后立即 `emit` 到 Socket.IO（见 4.4.1）。60 秒延迟显示与每秒平滑释放由前端实现。
...
后端在数据采集后**立即**将 `DataSnapshot` 推送给前端（无延迟）。60 秒延迟显示与每秒平滑释放的逻辑由**前端**实现（见 `frontend-design.md`），后端仅保留一个最多 5 分钟的内存队列用于新连接补发。

**设计思路**：

```
ESP32 采样 (1s间隔)  →  后端每 30s 拉取一批 (~30 点) 并立即推送  →  前端延迟队列 (60s缓冲)  →  每秒释放  →  图表 (流畅渲染)
```

- ESP32 以 1 秒间隔采样，后端每 30 秒通过 `GET_BUFFER` 拉取一批（约 30 个点），逐条持久化后立即 `io.emit('data:new', snapshot)`
- 前端将快照放入 `pendingBuffer`，每秒由定时器将到达显示时间（采集时刻 + 60s）的快照移入 `dataBuffer`
- 前端在入队时按时间戳**排序 + 去重**，防止 socket 重连补发 / REST 补全与实时推送产生重复数据点
...
gantt
    ...
    section 后端推送 (立即)
    推送 A             :milestone, c1, 0, 1
    推送 B             :milestone, c2, 30, 1
    推送 C             :milestone, c3, 60, 1
    ...
    section 前端每秒释放
    显示 A             :milestone, d1, 60, 1
    显示 B             :milestone, d2, 90, 1
    显示 C             :milestone, d3, 120, 1
```

> 前端显示的数据始终滞后真实时间约 60 秒。对于灌溉系统而言，1 分钟延迟完全可接受——灌溉决策的时效性要求远低于秒级。

**边缘情况处理**：

| 场景       | 处理                                                                                        |
| ---------- | ------------------------------------------------------------------------------------------- |
| 前端刚连接 | 后端推送队列中已有的数据点（最多 5 分钟内）全部立即推送，前端放入延迟队列按时间释放         |
| 后端重启   | 后端队列为内存结构，重启后清空。前端在重连后通过 REST API 拉取最近 5 分钟的历史数据补全图表 |
| 重复数据   | 前端入队时按时间戳排序 + 去重（Map 覆盖同时间戳），杜绝重复数据点被重复显示                 |
...
| `start_time`     | STRING (nullable) | 时间阈值开始 (HH:mm)，为 null 表示不限制时间  |
| `end_time`       | STRING (nullable) | 时间阈值结束 (HH:mm)，为 null 表示不限制时间  |

> 湿度任务只能存在一个。创建第二个时返回 `DUPLICATE_HUMIDITY_TASK` 错误。
>
> **时间阈值**（可选）：当 `start_time` 和 `end_time` 均设置时，湿度低于 `low_threshold` **且** 当前时间在 `[start_time, end_time)` 窗口内才会启动灌溉。支持跨 0 点，如 `start_time="16:00"`, `end_time="08:00"` 表示每天 16:00 ~ 次日 08:00。
>
> **时间阈值仅限制灌溉的启动时机**：一旦灌溉已启动，即使后续超出时间窗口，也会继续灌溉直到湿度达到 `high_threshold` 为止，避免灌溉中途停止导致土壤干湿交替。若两者均为 null，则行为与原来一致——仅按湿度阈值全天候控制。
...
2. 扫描 humidity 任务:
   - idle: 若有健康传感器 && avg_moisture < low_threshold && 在时间窗口内(若设置) → running (开阀门)
   - running: avg_moisture > high_threshold → completed (关阀门)
     (注: 时间窗口仅限制启动，不强制停止已运行的灌溉)
   - 全传感器故障 → paused
...
| `POST` | `/api/sensors/:id/calibration/calculate` | —                     | `{ a, b }`            | 计算校准公式                     |
| `GET`  | `/api/sensors/:id/calibration`           | —                     | `{ points, formula }` | 查看校准状态与历史数据点         |

> `formula` 结构为 `{ a: number, b: number }`，对应公式 $y = a \ln(\frac{1000}{x}) + b$。
...
| 事件名                    | 触发时机                           | Payload                                 |
| ------------------------- | ---------------------------------- | --------------------------------------- |
| `data:new`                | 采集后立即推送 (前端延迟 60s 显示) | `DataSnapshot` (见 4.4.2)               |
...
| 4   | 校准公式计算 **延后到用户主动请求**                                          | 数据点和计算分离，允许用户逐步收集数据点、观察趋势后再决定是否应用公式。避免每新增一个点就重新拟合导致频繁变更                                                                                                                                                                                                             |
| 14  | 校准公式确定为 **对数回归** $y = a \ln(\frac{1000}{x}) + b$                  | 通过实验测定含水量与脉冲计数呈对数关系。令 $u = \ln(1000/x)$ 线性化后用最小二乘法求解 $a$、$b$。脉冲计数为 0 或 ≥1500 时转换结果为 null。公式中的常数 1000 为拟合参数，与有效范围无关；干燥土壤脉冲约 1200 仍在有效范围内。校准以探头浸泡在水中为 100% 基准，潮湿土壤中受电解质影响含水量可能略超 100%，不视为异常，不截断 | 2026-07-26 |
...
| 16  | 推送架构改为 **后端立即推送 + 前端延迟显示**                                 | 后端每秒定时器释放队列导致推送间隔不均匀、且与 REST 补全/重连补发叠加产生重复数据点。改为后端采集后立即 emit，前端 `pendingBuffer` 实现 60s 延迟 + 每秒定时释放 + 时间戳排序去重，图表更新节奏稳定且无重复                                                                                                                 | 2026-09-16 |
| 15  | 湿度任务支持 **可选时间阈值**（`start_time` / `end_time`），且仅限制启动时机 | 用户可能希望仅在特定时段（如夜间 16:00 ~ 次日 08:00）进行湿度驱动灌溉，避免白天蒸发快导致频繁启停。时间阈值为可选字段，未设置时行为不变，支持跨 0 点窗口。**关键设计：时间窗口仅限制灌溉启动**，一旦启动则持续到湿度达标为止，不会因超出窗口而中途停止灌溉，避免土壤干湿交替损伤植物                                       | 2026-08-03 |
